...
const defaultOptions = {
    ...
    autoRotate: true, // Automatically apply EXIF orientation correction
    keepIccProfile: false,  // Default to false for backward compatibility
};
...
// Helper function to generate cache key based on strategy with retry logic
...
// Core image processing function - updated to use persistent cache with retry logic
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
        try {
            ...
            let sharpPipeline = sharp(resolvedInputPath, {
                ...
                sequentialRead: true, // Better for development file changes
            });

            // Apply EXIF orientation correction only when needed
            if (options.autoRotate) {
                // Check if the image has EXIF orientation that needs correction
                const metadata = await sharpPipeline.metadata();
                if (metadata.orientation && metadata.orientation !== 1) {
                    // Only apply rotation if there's an actual orientation value that's not 1 (normal)
                    console.log(`Sharp Plugin: Applying EXIF orientation correction (orientation: ${metadata.orientation}) for ${path.basename(resolvedInputPath)}`);
            ...
            // Preserve ICC profile and other metadata if enabled
            if (options.keepIccProfile) {
                sharpPipeline = sharpPipeline.withMetadata();
            }

            // Process and save the image with atomic write
            const tempOutputPath = `${outputFilePath}.tmp`;
            ...
            // If all retries failed or it's a different kind of error, throw it
