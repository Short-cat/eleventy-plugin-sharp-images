    ...
    for (let attempt = 0; attempt < retries; attempt++) {
        try {
            ...
            if (attempt < retries - 1 && (error.code === "ENOENT" || error.code === "EACCES" || error.code === "EBUSY")) {
                console.log(`Sharp Plugin: File temporarily unavailable (${error.code}), retrying in ${50 * (attempt + 1)}ms: ${inputPath}`);
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
        try {
            ...
            if (
                ...
                console.log(`Sharp Plugin: File operation failed (${error.code || error.message}), retrying in ${delay}ms: ${resolvedInputPath}`);
