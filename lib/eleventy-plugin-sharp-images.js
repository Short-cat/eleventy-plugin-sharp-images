...
// Helper function to generate cache key based on file content + operations
async function generateCacheKey(inputPath, operations) {
    try {
        ...
        let keyData;
        if (isCI) {
            // In CI environments, use content hash instead of mtime for reliable caching
            ...
            // Locally, use mtime for faster cache key generation
            keyData = {
                path: inputPath,
                size: stats.size,
...
async function processImageWithSharp(config, options) {
    ...
    // Check if output file already exists - use different validation for CI vs local
    try {
        const outputStats = await fs.stat(outputFilePath);

        if (outputStats.size > 0) {
            if (isCI) {
                // In CI, if the output file exists and we have the same cache key,
                // it means the content hasn't changed, so we can use it
                processedImages.set(cacheKey, outputUrl);
                return outputUrl;
            } else {
                // Locally, check mtime as before for faster validation
                const sourceStats = await fs.stat(resolvedInputPath);
